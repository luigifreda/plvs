...
#include "FastFusionManager.h"

FastFusionManager::FastFusionManager(bool createMeshList): 
...
FastFusionManager::~FastFusionManager()
{
    //	_timer->stop();
    //	delete _timer;
    _fusionAlive = false;
    ...
    fprintf(stderr, "\nEnd of FastFusionManager Destructor.");
...
void FastFusionManager::enableLighting()
{
...
void FastFusionManager::disableLighting()
{
    _lightingInternal = false;
    glDisable(GL_LIGHTING);
}

void FastFusionManager::drawGridFrame(float ox, float oy, float oz, float sx, float sy, float sz)
{
    disableLighting();
...
void FastFusionManager::generateBuffers()
{
    glewInit();
...
void FastFusionManager::drawMeshPointer()
{
...
void FastFusionManager::drawMeshInterleaved()
{
...
void FastFusionManager::drawCameraFrustum
(
 CameraInfo pose,
...
void FastFusionManager::setScenePosition(CameraInfo pose)
{
    CameraInfo inv = pose;
...
void FastFusionManager::fastDraw()
{
...
//void FastFusionManager::keyPressEvent(QKeyEvent *e)
//{
...
void FastFusionManager::updateSlot()
{
    if (_threadFusion)
    {
        if (_poses.size() == 1)
        ...
        if (_currentFrame < _nextStopFrame)
        {
            ...
            if (_currentFrame < (long long) (_depthNames[_currentTrajectory].size()))
            {
                if (_currentFrame > _lastComputedFrame)
                {
                    if (_poses.size() <= 1)
                    {
                        std::cout << "depth name: " << _depthNames[_currentTrajectory][_currentFrame] << std::endl;
