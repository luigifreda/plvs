...
int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPointPtr> &vpMapPoints, const float th)
{
    int nmatches=0;
...
bool ORBmatcher::CheckDistEpipolarLine(const cv::KeyPoint &kp1,const cv::KeyPoint &kp2,const cv::Mat &F12,const KeyFramePtr& pKF2)
{
    ...
    return dsqr<3.84*pKF2->mvLevelSigma2[kp2.octave]; // value of inverse cumulative chi-square for 1 DOF (Hartley Zisserman pag 567)
}
...
int ORBmatcher::SearchByProjection(KeyFramePtr& pKF, cv::Mat Scw, const vector<MapPointPtr> &vpPoints, vector<MapPointPtr> &vpMatched, int th)
{
...
int ORBmatcher::SearchForTriangulation(KeyFramePtr& pKF1, KeyFramePtr& pKF2, const cv::Mat& F12,
    ...
    for(int i=0; i<nMPs; i++)
    {
        ...
        for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
        {
            ...
            if(pKF->mvuRight[idx]>=0)
            {
                ...
                if(e2*pKF->mvInvLevelSigma2[kpLevel]>7.8) // 7.81 = value of the inverse cumulative chi-square with 3 DOF (Hartley-Zisserman pag 567) 
                    continue;
            }
            else
            {
                ...
                if(e2*pKF->mvInvLevelSigma2[kpLevel]>5.99) // 5.99 = value of the inverse cumulative chi-square with 2 DOF (Hartley-Zisserman pag 567)  
                    continue;
            }
        ...
        if(bestDist<=TH_LOW)
        {
            ...
            {
                if( pMP->AddObservation(pKF,bestIdx) )
...
int ORBmatcher::SearchBySim3(KeyFramePtr& pKF1, KeyFramePtr& pKF2, vector<MapPointPtr> &vpMatches12,
