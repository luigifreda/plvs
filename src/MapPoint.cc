...
bool MapPoint::AddObservation(const KeyFramePtr& pKF, size_t idx)
{
    unique_lock<mutex> lock(mMutexFeatures);
...
//        {
//            pMP->AddObservation(pKF,mit->second);  // N.B.: here we are guaranteed that the observation is added since we have preliminary checked is not in keyframe
        if(pMP->AddObservation(pKF,mit->second))
        {
...
int MapPoint::GetIndexInKeyFrame(const KeyFramePtr& pKF)
{
    unique_lock<mutex> lock(mMutexFeatures);
#if !ENABLE_NEW_CHANGES    
    if(mObservations.count(pKF))
