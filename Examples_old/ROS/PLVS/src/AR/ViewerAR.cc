...
void ViewerAR::Run()
{
    ...
    wui = 20* pangolin::default_font().MaxWidth();
...
void ViewerAR::SetImagePose(const cv::Mat &im, const cv::Mat &Tcw, const int &status, const vector<cv::KeyPoint> &vKeys, const vector<PLVS2::MapPoint*> &vMPs)
{
...
Plane* ViewerAR::DetectPlane(const cv::Mat Tcw, const std::vector<MapPoint*> &vMPs, const int iterations)
{
    ...
    for(size_t i=0; i<vMPs.size(); i++)
    {
        ...
        if(pMP)
        {
            ...
            {
                vPoints.push_back(Converter::toCvMat(pMP->GetWorldPos()));
...
void Plane::Recompute()
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(!pMP->isBad())
        {
            cv::Mat Xw = Converter::toCvMat(pMP->GetWorldPos());
            o+=Xw;
