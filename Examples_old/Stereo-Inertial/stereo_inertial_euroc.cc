...
int main(int argc, char **argv)
{
    ...
    PLVS2::System SLAM(argv[1],argv[2],PLVS2::System::IMU_STEREO, bUseViewer);
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        for(int ni=0; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            if(ni>0)
                ...
                {
                    vImuMeas.push_back(PLVS2::IMU::Point(vAcc[seq][first_imu[seq]].x,vAcc[seq][first_imu[seq]].y,vAcc[seq][first_imu[seq]].z,
    ...
    cout << "perc images lost: " << (float(numImgsLost)/tot_images)*100. << std::endl; 
    cout << "perc images no init: " << (float(numImgsNoInit)/tot_images)*100. << std::endl;  

    Logger logger("Performances.txt");
    logger << "perc images lost: " << (float(numImgsLost)/tot_images)*100. << std::endl; 
    logger << "perc images no init: " << (float(numImgsNoInit)/tot_images)*100. << std::endl; 
    logger << "median tracking time: " << vTimesTrack[tot_images/2] << endl;
    logger << "mean tracking time: " << totaltime/tot_images << endl;
