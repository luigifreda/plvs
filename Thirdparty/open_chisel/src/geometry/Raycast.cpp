...
inline float mod(const float value, const float modulus)
{
  return fmod(fmod(value, modulus) + modulus,  modulus);
}

inline float intbound(float s, const float ds)
{
...
void Raycast(const Vec3& start, const Vec3& end, const Point3& min, const Point3& max, Point3List* output)
{
    ...
    const int endX = (int)std::floor(end.x());
    const int endY = (int)std::floor(end.y());
    const int endZ = (int)std::floor(end.z());
    
    const Vec3 direction = (end - start);
    const float maxDist = direction.squaredNorm();

    // Break out direction vector.
    const float dx = endX - x;
    const float dy = endY - y;
    const float dz = endZ - z;

    // Direction to increment x,y,z when stepping.
    const int stepX = (int)signum((int)dx);
    const int stepY = (int)signum((int)dy);
    const int stepZ = (int)signum((int)dz);
