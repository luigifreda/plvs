...
void TsdfIntegratorBase::integrateWorlPointCloud(const Transformation& T_G_C,
                                   const Pointcloud& points_C,
                                   const Colors& colors,
                                   const Normals& normals) {

  size_t point_idx;
  for (point_idx = 0; point_idx < points_C.size(); ++point_idx) {
    const Point& point_C = points_C[point_idx];
    const Color& color = colors[point_idx];
    const Normal& normal_C = normals[point_idx].normalized();

    const Point origin = T_G_C.getPosition();
    const Point point_G = T_G_C * point_C;
    const Ray normal_G = T_G_C * normal_C;
    
    const Ray& unit_ray = normal_G;
    
    const Point ray_start = point_G + unit_ray*config_.default_truncation_distance;
    const Point ray_end = point_G - unit_ray*config_.default_truncation_distance;    
...
//                         config_.default_truncation_distance);
    ...
    //const Point start_scaled = point_G * voxel_size_inv_;
    //VoxelIndex global_voxel_idx = getGridIndexFromPoint(start_scaled);
    ...
    while (ray_caster.nextRayIndex(&global_voxel_idx)) 
    {
      ...
      //const float weight = getVoxelWeight(point_G-ray_start); // replace camera origin with ray_start 
      const float weight = 1; 

      //updateTsdfVoxel(origin, point_G, global_voxel_idx, color, weight, voxel);
      updateTsdfVoxel(ray_start, point_G, global_voxel_idx, color, weight, voxel);
    }
  }
}
