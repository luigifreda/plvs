...
std::shared_ptr<PLVS::System> pSLAM;

std::shared_ptr<PLVS::PointCloudMapping> pPointCloudMapping;
ros::Publisher pointCloudPublisher;
pcl::uint64_t map_timestamp = 0;

int trackingState = (int)PLVS::Tracking::NOT_INITIALIZED;
...
public:
    ...
    void SetSlamSystem(PLVS::System* pSLAM)
    {
        mpSLAM = pSLAM;
    }
...
void pointcloudCallback(const ros::TimerEvent& event)
{
    ...
    {
        pcl::uint64_t current_timestamp = pPointCloudMapping->GetMapTimestamp();
        if ( (pointCloudPublisher.getNumSubscribers() > 0) && (current_timestamp > map_timestamp) )
        {
            map_timestamp = current_timestamp;
            PLVS::PointCloudMapping::PointCloudT::Ptr pMap = pPointCloudMapping->GetMap();
...
int main(int argc, char **argv)
{
    ...
    bUseViewer = static_cast<int> (PLVS::Utils::GetParam(fSettings, "Viewer.on", 1)) != 0;
    bWaitForCameraInfo = static_cast<int> (PLVS::Utils::GetParam(fSettings, "Camera.waitCameraInfoOn", 0)) != 0;
    ...
    pSLAM = std::make_shared<PLVS::System >(argv[1], argv[2], PLVS::System::RGBD, bUseViewer);
...
void ImageGrabber::SendTransform(const ros::Time& time)
{
    ...
    tf::Vector3 cameraTranslation(T_wo.at<float>(0, 3), T_wo.at<float>(1, 3), T_wo.at<float>(2, 3));
...
void ImageGrabber::GrabRGBD(const sensor_msgs::ImageConstPtr& msgRGB, const sensor_msgs::ImageConstPtr& msgD)
{
    ...
    if ( cv_ptrRGB->image.channels() == 4 ) 
    {
        ...
        if (!bRGB)
        {
            cv::cvtColor(convertedImg, convertedImg, CV_RGBA2BGR, 3);
        }  
        else
        {
            cv::cvtColor(convertedImg, convertedImg, CV_RGBA2RGB, 3);                 
        }
    }   
    else 
    {
    ...
    cv::Mat T_ow = mpSLAM->TrackRGBD(convertedImg, cv_ptrD->image, cv_ptrRGB->header.stamp.toSec());
