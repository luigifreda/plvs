...
{
public:
    Plane(const std::vector<MapPointPtr> &vMPs, const cv::Mat &Tcw);
...
public:
    ...
    void SetSLAM(PLVS::System* pSystem){
        mpSystem = pSystem;
    }
    ...
    void SetImagePose(const cv::Mat &im, const cv::Mat &Tcw, const int &status,
                      const std::vector<cv::KeyPoint> &vKeys, const std::vector<MapPointPtr> &vMPs);

    void GetImagePose(cv::Mat &im, cv::Mat &Tcw, int &status,
                      std::vector<cv::KeyPoint> &vKeys,  std::vector<MapPointPtr> &vMPs);

private:

    //SLAM
    PLVS::System* mpSystem;
    ...
    void DrawTrackedPoints(const std::vector<cv::KeyPoint> &vKeys, const std::vector<MapPointPtr> &vMPs, cv::Mat &im);

    Plane* DetectPlane(const cv::Mat Tcw, const std::vector<MapPointPtr> &vMPs, const int iterations=50);
