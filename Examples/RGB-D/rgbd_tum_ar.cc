...
void InitViewerAR(cv::FileStorage& fSettings, PLVS::ViewerAR& viewerAR, cv::Mat& K, cv::Mat& DistCoef, bool& bRGB);
void updateViewerAR(PLVS::ViewerAR& viewerAR, PLVS::System& SLAM, cv::Mat& im, cv::Mat& Tcw, cv::Mat& K, cv::Mat& DistCoef, bool bRGB);
...
int main(int argc, char **argv) 
{
    ...
    bool bUseViewer = false;//static_cast<int> (PLVS::Utils::GetParam(fSettings, "Viewer.on", 1)) != 0;
        
    // Create SLAM system. It initializes all system threads and gets ready to process frames.
    PLVS::System SLAM(argv[1],argv[2],PLVS::System::RGBD,bUseViewer);
    
    
    PLVS::ViewerAR viewerAR;
    viewerAR.SetSLAM(&SLAM);        
    bool bRGB = true;
    cv::Mat K;
    cv::Mat DistCoef;     
    InitViewerAR(fSettings, viewerAR, K, DistCoef, bRGB);    
    thread tViewer = thread(&PLVS::ViewerAR::Run,&viewerAR);        
    ...
    for(int ni=0; ni<nImages; ni++)
    {
        ...
        updateViewerAR(viewerAR, SLAM, imRGB, Tcw, K, DistCoef, bRGB);        
...
void updateViewerAR(PLVS::ViewerAR& viewerAR, PLVS::System& SLAM, cv::Mat& imRGB, cv::Mat& Tcw, cv::Mat& K, cv::Mat& DistCoef, bool bRGB)
{    
    cv::Mat im = imRGB.clone();
    cv::Mat imu;
    int state = SLAM.GetTrackingState();
    vector<PLVS::MapPointPtr> vMPs = SLAM.GetTrackedMapPoints();
